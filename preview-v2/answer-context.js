...
export function prepareQuestion(question,history=[],clock=todayContext()){
  ...
  let team=explicit || (followUp?[...users].reverse().map(m=>teamIn(m.content)).find(Boolean):"") || "";
  ...
  sources.forEach((s,index)=>{
    ...
    for(let n=0;n<tokens.length;n++){
      ...
      for(let t=0;t<times.length;t++){
        ...
        out.push({date,time:time[1].padStart(2,"0")+":"+time[2],opponent,index,
          argentina:/hora\s*\(ARG\)|hora(?:rio)?\s+(?:de\s+)?argentina|hora argentina/i.test(s.title+" "+text)});
      }
    }
  });
